...
export default function PreinscripcionesTable({ preinscripciones, onView, onApprove, onObserve, onReject, loading }) {
  if (loading) {
          ...
          {preinscripciones.map((item) => {
            ...
            return (
              <tr key={item.id}>
                ...
                <td><EstadoPreinscripcionBadge estado={item.estado} /></td>
                <td>{fecha}</td>
                <td>
                  <div className="actions-cell">
                    ...
                    <button className="icon-btn" type="button" title="Aprobar solicitud" onClick={() => onApprove(item)}>
                      <CheckCircle2 size={18} />
                    </button>
                    <button className="icon-btn" type="button" title="Observar solicitud" onClick={() => onObserve(item)}>
